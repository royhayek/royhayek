export const t = {
  fr: {
    // Navbar
    nav_about: "À propos",
    nav_portfolio: "Portfolio",
    nav_experience: "Expérience",
    nav_contact: "Contact",
    nav_resume: "Télécharger mon CV",
    nav_resume_short: "CV",

    // Hero
    hero_badge: "Disponible à partir de juillet 2026",
    hero_bio_1: "Ingénieur Logiciel & IA avec 5 ans d'expérience en développement frontend, mobile et IA. Actuellement en M2 IA à l'ECE Paris,",
    hero_bio_highlight: "disponible à partir de juillet 2026",
    hero_bio_2: "pour de nouvelles opportunités en Île-de-France et au-delà.",
    hero_cta: "Voir mes projets",
    hero_scroll: "Défiler",
    hero_linkedin: "LinkedIn",

    // Typewriter
    type: [
      "Ingénieur Logiciel & IA", 2500,
      "Développeur Frontend", 2500,
      "Ingénieur IA", 2500,
      "Expert React & React Native", 2500,
    ],

    // About
    about_label: "À propos de moi",
    about_title_1: "Créer des expériences qui",
    about_title_highlight: "séduisent autant",
    about_title_2: "qu'elles fonctionnent",
    about_p1: "Ingénieur Logiciel & IA basé à Boulogne-Billancourt (92), avec plus de 5 ans d'expérience dans la création d'applications grand public pour des startups et des entreprises établies au Liban, en Arabie Saoudite et au-delà, dont un rôle de Lead Frontend dans une fintech à fort trafic.",
    about_p2: "Actuellement en M2 Intelligence Artificielle à l'ECE Paris, je combine une solide expertise React / React Native avec des compétences en Machine Learning, NLP, LLMs, RAG et systèmes agentiques.",
    about_tech: "Stack technique",

    // Portfolio
    portfolio_label: "Travaux sélectionnés",
    portfolio_title: "Portfolio",
    portfolio_desc: "Projets React & React Native que j'ai construits ou auxquels j'ai contribué : du digital wallet et du trading d'actions à la planification de voyages par IA, en passant par des apps de fidélité. Cliquez sur une carte pour ouvrir le projet.",
    portfolio_view: "Voir le projet",

    // Experience
    exp_label: "Expérience professionnelle",
    exp_title: "Parcours",
    exp_desc: "Plus de 5 ans à concevoir et livrer des produits dans la fintech, le voyage et la restauration, du Liban à l'Arabie Saoudite.",

    experiences: [
      {
        role: "Ingénieur Frontend Senior / Lead Frontend",
        company: "Cube Holdings Ltd",
        location: "Beyrouth, Liban",
        cityLabel: "Beyrouth",
        period: "Sept 2024 - Fév 2026",
        bullets: [
          "Direction de l'équipe frontend : recrutement, mentorat, revues de code, définition des standards et bonnes pratiques d'ingénierie.",
          "Gestion des processus Agile / Scrum : backlog, planification des sprints, démos et rétrospectives.",
          "Développement d'une application digital wallet (React Native / Expo, TypeScript) avec authentification sécurisée (access / refresh tokens) et intégration d'APIs.",
          "Conception de dashboards d'administration et de systèmes de réconciliation financière (React / Next.js) en support de la plateforme digital wallet.",
          "Amélioration de la qualité via tests automatisés (Jest, React Testing Library, Playwright), optimisations de performance et déploiements automatisés (EAS mobile et web).",
          "Développement d'un chatbot RAG (IntelliConvo) : pipeline complet (embeddings, Pinecone, OpenAI), déployé en Next.js / PostgreSQL sur Vercel et Railway.",
        ],
      },
      {
        role: "Ingénieur Frontend Senior",
        company: "Aljazira Capital",
        location: "Riyad, Arabie saoudite",
        cityLabel: "Riyad",
        period: "Juil 2022 - Juil 2024",
        bullets: [
          "Contribution au développement d'une plateforme de trading d'actions (React Native + React.js), l'un des projets les plus exigeants de mon parcours.",
          "Traduction de wireframes Figma en interfaces responsives prêtes pour la production : écrans de trading, portefeuilles et flux de transactions.",
          "Implémentation du streaming temps réel des prix de marché et des mises à jour de portefeuille à haute fréquence.",
          "Optimisation des performances via profiling, rendering et gestion d'état efficace pour supporter des mises à jour haute fréquence.",
          "Intégration d'APIs REST avec TypeScript au sein d'une équipe Agile avec designers, backend engineers, BAs et QA.",
        ],
      },
      {
        role: "Ingénieur Logiciel",
        company: "CME",
        location: "Beyrouth, Liban",
        cityLabel: "Beyrouth",
        period: "Sept 2021 - Juil 2022",
        bullets: [
          "Livraison de plusieurs applications mobiles et web en production pour différents clients.",
          "Développement et maintenance d'applications React Native pour des chaînes de restauration reconnues au Liban, nouvelles fonctionnalités et correction de bugs en production.",
          "Développement d'une plateforme web React.js liée à Vision 2030 (Arabie saoudite) : composants réutilisables et intégration d'APIs REST.",
          "Collaboration Agile (Scrum) avec designers, backend engineers, BAs et QA.",
        ],
      },
      {
        role: "Développeur Full Stack",
        company: "Tac-Techs",
        location: "Koweït (remote)",
        cityLabel: "Koweït",
        period: "Janv 2020 - Sept 2021",
        bullets: [
          "Développement d'applications mobiles cross-platform avec Flutter et de systèmes backend avec Laravel, solutions full-stack complètes.",
          "Conception et développement d'APIs REST avec Node.js et Laravel, intégrées dans plusieurs applications mobiles et web.",
          "Intégration d'APIs externes diverses, assurant un échange de données fluide et l'interopérabilité entre systèmes.",
          "Mise en place de pipelines CI/CD avec GitHub Actions, déploiement App Store / Google Play et hébergement en production.",
        ],
      },
      {
        role: "Développeur Android (Stage)",
        company: "Monty Mobile",
        location: "Beyrouth, Liban",
        cityLabel: "Beyrouth",
        period: "Avr 2016 - Mai 2016",
        bullets: [
          "Développement d'applications Android natives performantes et réutilisables avec Kotlin, à partir de maquettes et wireframes.",
          "Identification et résolution de goulots d'étranglement de performance et de bugs.",
          "Collaboration avec les équipes pour définir et implémenter de nouvelles fonctionnalités.",
          "Intégration de services backend et d'APIs, assurant un échange de données fluide et de bonnes performances applicatives.",
        ],
      },
    ],

    // Contact form
    contact_label: "Entrer en contact",
    contact_title: "Discutons",
    contact_desc: "Vous avez un projet en tête ou envie de collaborer ? Envoyez-moi un message, je vous répondrai rapidement.",
    contact_name: "Nom",
    contact_name_placeholder: "Votre nom",
    contact_email: "E-mail",
    contact_email_placeholder: "votre@email.com",
    contact_message: "Message",
    contact_message_placeholder: "Parlez-moi de votre projet…",
    contact_send: "Envoyer le message",
    contact_sending: "Envoi en cours…",
    contact_success_title: "Message envoyé !",
    contact_success_desc: "Merci pour votre message. Je vous répondrai dans les plus brefs délais.",
    contact_error: "Une erreur est survenue. Veuillez réessayer.",
    contact_send_another: "Envoyer un autre message",

    // Footer
    footer_role: "Ingénieur Logiciel & IA · France",
    footer_copy: "Portfolio de Roy",

    // Projects
    projects: [
      {
        title: "Buitanda - Online Store Angola",
        description: "Projet freelance développé avec React Native pour un client en Angola. Application e-commerce avec plus de 10 000 utilisateurs actifs, proposant des milliers de produits (électronique, mode, beauté, maison) avec livraison rapide et paiement sécurisé.",
      },
      {
        title: "Mojo Wallet App",
        description: "Lead Frontend chez Cube Holdings. Direction de l'équipe et développement d'une application digital wallet (React Native / Expo, TypeScript) avec authentification sécurisée, dashboards d'administration (Next.js) et chatbot RAG (IntelliConvo) intégrant embeddings, Pinecone et OpenAI.",
      },
      {
        title: "Aljazira Tadawul — Web",
        description: "Site de trading boursier développé avec React pour Aljazira Capital (Arabie Saoudite). Données de marché en temps réel, graphiques interactifs et transactions sécurisées.",
      },
      {
        title: "Aljazira Tadawul — Mobile",
        description: "Application de trading créée avec React Native pour Aljazira Capital. Mises à jour boursières en temps réel et outils de trading avancés pour la gestion de portefeuille en mobilité.",
      },
      {
        title: "Hello Curious",
        description: "Projet freelance développé avec React Native. Permet de découvrir et débloquer des expériences à vivre seul ou en famille et entre amis.",
      },
      {
        title: "Avandra — Planificateur de voyages IA",
        description: "Projet personnel développé avec React Native. Simplifie la planification de voyages grâce à l'IA pour créer des itinéraires complets vers n'importe quelle destination.",
      },
      {
        title: "CMS pour Avandra",
        description: "Système de gestion de contenu pour l'application Avandra AI Trip Planner. Analyses, gestion du contenu en temps réel et suivi de l'engagement utilisateur.",
      },
      {
        title: "Roadster Diner",
        description: "Contribution à l'app Roadster Diner en React Native : programme de fidélité, roue de la fortune et paiement par portefeuille.",
      },
      {
        title: "Deek Duke",
        description: "Contribution à l'app Deek Duke en React Native : paiement par portefeuille, système de coupons et programme de fidélité.",
      },
      {
        title: "Bartartine",
        description: "Contribution à l'app Bartartine en React Native : fidélité, paiement par portefeuille, corrections de bugs et améliorations de performances.",
      },
      {
        title: "Zaatar w Zeit",
        description: "Contribution à l'app Zaatar w Zeit (LB & KSA) en React Native : paiement par portefeuille, corrections de bugs et améliorations de performances.",
      },
    ],
  },

  en: {
    // Navbar
    nav_about: "About",
    nav_portfolio: "Portfolio",
    nav_experience: "Experience",
    nav_contact: "Contact",
    nav_resume: "Download Resume",
    nav_resume_short: "Resume",

    // Hero
    hero_badge: "Available from July 2026",
    hero_bio_1: "Software & AI Engineer with 5+ years of experience in frontend, mobile and AI development. Currently pursuing an M.Sc. in AI at ECE Paris,",
    hero_bio_highlight: "available from July 2026",
    hero_bio_2: "for exciting new opportunities in France and beyond.",
    hero_cta: "View My Work",
    hero_scroll: "Scroll",
    hero_linkedin: "LinkedIn",

    // Typewriter
    type: [
      "Software & AI Engineer", 2500,
      "Frontend Developer", 2500,
      "AI Engineer", 2500,
      "React & React Native Dev", 2500,
    ],

    // About
    about_label: "About me",
    about_title_1: "Crafting experiences that",
    about_title_highlight: "feel as good",
    about_title_2: "as they look",
    about_p1: "I'm a Software & AI Engineer based in Boulogne-Billancourt, France, with 5+ years of experience building high-traffic consumer apps for startups and established companies across Lebanon, KSA, and beyond, including a Frontend Tech Lead role at a fintech company.",
    about_p2: "Currently pursuing an M.Sc. in Artificial Intelligence at ECE Paris, I combine deep React / React Native expertise with skills in Machine Learning, NLP, LLMs, RAG and agentic AI systems.",
    about_tech: "Tech Stack",

    // Portfolio
    portfolio_label: "Selected work",
    portfolio_title: "Portfolio",
    portfolio_desc: "React & React Native projects I've built or contributed to, from fintech digital wallets and stock trading platforms to AI trip planning and restaurant loyalty apps. Tap any card to open the live project.",
    portfolio_view: "View project",

    // Experience
    exp_label: "Professional Experience",
    exp_title: "Work History",
    exp_desc: "5+ years designing and shipping products across fintech, travel and hospitality, from Lebanon to Saudi Arabia.",

    experiences: [
      {
        role: "Senior Frontend Engineer / Frontend Tech Lead",
        company: "Cube Holdings Ltd",
        location: "Beirut, Lebanon",
        cityLabel: "Beirut",
        period: "Sep 2024 - Feb 2026",
        bullets: [
          "Led the frontend team: recruitment, mentoring, code reviews and defining engineering standards and best practices.",
          "Managed Agile / Scrum processes: backlog grooming, sprint planning, demos and retrospectives.",
          "Built a digital wallet app (React Native / Expo, TypeScript) with secure authentication (access / refresh tokens) and API integrations.",
          "Designed and built admin dashboards and financial reconciliation systems (React / Next.js) supporting the digital wallet platform.",
          "Improved quality through automated testing (Jest, React Testing Library, Playwright), performance optimisation and automated deployments (EAS mobile and web).",
          "Developed a RAG chatbot (IntelliConvo): full pipeline (embeddings, Pinecone, OpenAI), deployed on Next.js / PostgreSQL via Vercel and Railway.",
        ],
      },
      {
        role: "Senior Frontend Engineer",
        company: "Aljazira Capital",
        location: "Riyadh, Saudi Arabia",
        cityLabel: "Riyadh",
        period: "Jul 2022 - Jul 2024",
        bullets: [
          "Contributed to a stock trading platform (React Native + React.js), one of the most technically demanding projects of my career.",
          "Translated Figma wireframes into responsive, production-ready interfaces: trading screens, portfolio views and transaction flows.",
          "Implemented features handling live market price streaming and real-time high-frequency portfolio updates.",
          "Optimised performance through profiling, rendering and efficient state management to support high-frequency updates.",
          "Integrated REST APIs with TypeScript and collaborated in Agile (Scrum) with designers, backend engineers, BAs and QA.",
        ],
      },
      {
        role: "Software Engineer",
        company: "CME",
        location: "Beirut, Lebanon",
        cityLabel: "Beirut",
        period: "Sep 2021 - Jul 2022",
        bullets: [
          "Delivered multiple client-facing mobile and web applications across different industries.",
          "Built and maintained React Native apps for leading restaurant chains in Lebanon, shipping new features and resolving production bugs.",
          "Built a React.js web platform for Saudi Arabia's Vision 2030 initiative: reusable components and REST API integrations within a cross-functional team.",
          "Collaborated in Agile (Scrum) with designers, backend engineers, BAs and QA.",
        ],
      },
      {
        role: "Full Stack Developer",
        company: "Tac-Techs",
        location: "Kuwait (remote)",
        cityLabel: "Kuwait City",
        period: "Jan 2020 - Sep 2021",
        bullets: [
          "Developed cross-platform mobile apps with Flutter and backend systems with Laravel, delivering complete full-stack solutions.",
          "Designed and built REST APIs with Node.js and Laravel, integrated across multiple mobile and web applications.",
          "Integrated diverse external APIs, enabling seamless data exchange and system interoperability.",
          "Set up and managed CI/CD pipelines with GitHub Actions, deploying to App Store / Google Play and hosting in production.",
        ],
      },
      {
        role: "Android Developer (Internship)",
        company: "Monty Mobile",
        location: "Beirut, Lebanon",
        cityLabel: "Beirut",
        period: "Apr 2016 - May 2016",
        bullets: [
          "Developed high-performance, reusable native Android applications with Kotlin from wireframes and mockups.",
          "Identified and resolved performance bottlenecks and bugs.",
          "Collaborated with teams to define and implement new features.",
          "Integrated backend services and APIs, ensuring smooth data exchange and strong application performance.",
        ],
      },
    ],

    // Contact form
    contact_label: "Get in touch",
    contact_title: "Let's Talk",
    contact_desc: "Have a project in mind or want to collaborate? Send me a message and I'll get back to you promptly.",
    contact_name: "Name",
    contact_name_placeholder: "Your name",
    contact_email: "Email",
    contact_email_placeholder: "you@email.com",
    contact_message: "Message",
    contact_message_placeholder: "Tell me about your project…",
    contact_send: "Send Message",
    contact_sending: "Sending…",
    contact_success_title: "Message sent!",
    contact_success_desc: "Thanks for reaching out. I'll get back to you as soon as possible.",
    contact_error: "Something went wrong. Please try again.",
    contact_send_another: "Send another message",

    // Footer
    footer_role: "Software & AI Engineer · France",
    footer_copy: "Roy's Portfolio",

    // Projects
    projects: [
      {
        title: "Buitanda - Online Store Angola",
        description: "Freelance e-commerce app built with React Native for a client in Angola. 10k+ active users, offering thousands of products across electronics, fashion, beauty and home categories, with fast local delivery and secure payments.",
      },
      {
        title: "Mojo Wallet App",
        description: "Frontend Tech Lead at Cube Holdings. Led the team and built a digital wallet app (React Native / Expo, TypeScript) with secure auth, admin dashboards (Next.js), financial reconciliation systems, and a RAG chatbot (IntelliConvo) using embeddings, Pinecone and OpenAI.",
      },
      {
        title: "Aljazira Tadawul — Web",
        description: "A stock trading website built with React for Aljazira Capital, KSA. Real-time market data, interactive charts, and secure trading functionalities.",
      },
      {
        title: "Aljazira Tadawul — Mobile",
        description: "A stock trading app built with React Native for Aljazira Capital. Real-time stock updates and advanced trading tools for on-the-go portfolio management.",
      },
      {
        title: "Hello Curious",
        description: "Freelance project built with React Native. Lets people discover and unlock experiences to enjoy alone or with family and friends.",
      },
      {
        title: "Avandra — AI Trip Planner",
        description: "Personal project built with React Native. Simplifies travel planning using AI to create complete itineraries for any destination.",
      },
      {
        title: "CMS for Avandra",
        description: "A CMS for the Avandra AI Trip Planner with analytics, real-time content management, and user engagement tracking.",
      },
      {
        title: "Roadster Diner",
        description: "Contributed to the Roadster Diner app using React Native: loyalty feature, spinning wheel, and wallet-based bill payment.",
      },
      {
        title: "Deek Duke",
        description: "Contributed to the Deek Duke app using React Native: wallet-based bill payment, coupon system, and loyalty feature.",
      },
      {
        title: "Bartartine",
        description: "Contributed to the Bartartine app using React Native: loyalty feature, wallet-based payment, bug fixes, and performance improvements.",
      },
      {
        title: "Zaatar w Zeit",
        description: "Contributed to the Zaatar w Zeit app (LB & KSA) using React Native: wallet-based bill payment, bug fixes, and performance improvements.",
      },
    ],
  },
};
